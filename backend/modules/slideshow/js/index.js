...
jsBackend.slideshow =
{
		...
		if($('tr.noSlideshow').length > 0)
		{
			...
			$('table.dataGrid').droppable(
			{
				...
				{	
					...
					$(this).find('tr.noSlideshow').remove();
				}
			});
		}
	},
	 ...
	 */
	bindDragAndDropCategorySlideshow: function()
	{	
		...
		$.each($('div.dataGridHolder'), function()
		{
			...
			$('div.dataGridHolder').sortable(
			{
				...
				{
					// vars we will need
					var slideshowId = ui.item.attr('id');
					...
					$.ajax(
					{
						...
						url: '/backend/ajax.php?module=' + jsBackend.current.module + '&action=sequence_slideshow&language=' + jsBackend.current.language,
						data: 'slideshowId=' + modelId + '&fromCategoryId=' + fromCategoryId + '&toCategoryId=' + toCategoryId + '&fromCategorySequence=' + fromCategorySequence + '&toCategorySequence=' + toCategorySequence,
						...
						{ 
							...
							{ 
								...
								{
									$('div#dataGrid-' + fromCategoryId + ' table.dataGrid').append('<tr class="noSlideshow"><td colspan="3">{$msgNoSlideshowInCategory}</td></tr>');
								}
...
$(document).ready(jsBackend.slideshow.init);